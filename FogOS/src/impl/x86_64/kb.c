...
#define MAX_INPUT_LENGTH 1024 // Maximum length of the input string
...
string EDITOR_readline()
{
    ...
    while (1)
    {
        ...
        if (scancode == 0x01) // Escape key (exit editor)
        {
            printc('\n');
            return inputBuffer;
        }
        ...
        // Handle Enter key (return the input when pressed)
        if (scancode == 0x1C) // Enter key
        {
            inputBuffer[i++] = '\n';
            printc('\n');
        ...
        if (scancode == 0x0E) // Backspace key
        {
            ...
            {
                i--;
                printc('\b'); // Print backspace to overwrite the character
            }
            continue;
        }
        ...
        if (scancode < sizeof(characterTable))
        {
            ...
            if (character)
            {
                inputBuffer[i++] = character; // Append character to the buffer
                printc(character);            // Print the character
                if (i >= MAX_INPUT_LENGTH - 1)
                {
                    inputBuffer[i] = '\0'; // Null-terminate the string
                    printc('\n');
                    return inputBuffer; // Return the input
                }
            }
        }
    }
